window.addEventListener('DOMContentLoaded', () => {
    ...
    // ============================================================
    // 1. КОНФИГУРАЦИЯ SVG (ФОРМА МОЗГА)
    // ============================================================
    ...
    // ============================================================
    // 2. ИДЕАЛЬНЫЙ ТАЙМИНГ (БЕСШОВНЫЙ ЛУП)
    // ============================================================
    const ANIMATION_DURATION = 20;     
    
    // ГЛОБАЛЬНЫЕ ФАЗЫ (Прозрачность всей сцены)
    const FADE_IN_END = 2;              // 0-2 сек: Появление из пустоты
    const GLOBAL_FADE_OUT_START = 17;   // 17-20 сек: Уход в пустоту
    
    // ФАЗЫ МОЗГА (Внутренняя структура)
    const PHASE_CONNECTION_END = 5;     // 2-5 сек: Прорастание линий
    const PHASE_FORMATION_END = 8;      // 5-8 сек: Формирование плотной структуры
    
    // ВАЖНО: Структура мозга начинает распадаться РАНЬШЕ, чем исчезает сцена.
    // Это возвращает частицы к хаосу, чтобы конец соответствовал началу.
    const BRAIN_DISSOLVE_START = 15;    // 15-20 сек: Линии и форма мозга исчезают
    ...
    class Particle {
        constructor(x, y) { 
            ...
            this.vy = (Math.random() - 0.5) * DRIFT_SPEED * 2; 
        }
        update() { 
            this.x += this.vx; 
            this.y += this.vy; 
            // Бесконечное поле (wrap around)
    ...
    function animate() {
        ...
        // --- РАСЧЕТ ПРОЗРАЧНОСТИ (ГЛАВНЫЙ ЦИКЛ) ---
        // Появление в начале (0 -> 1)
        const fadeIn = calculatePhaseProgress(0, FADE_IN_END, animationTime);
        // Исчезновение в конце (0 -> 1, потом инвертируем)
        const fadeOut = calculatePhaseProgress(GLOBAL_FADE_OUT_START, ANIMATION_DURATION, animationTime);
        // Итоговая прозрачность сцены
        const masterOpacity = fadeIn * (1 - fadeOut);

        // --- РАСЧЕТ СТРУКТУРЫ МОЗГА ---
        // Формирование (0 -> 1)
        const connectionProgress = calculatePhaseProgress(FADE_IN_END, PHASE_CONNECTION_END, animationTime);
        const formationProgress = calculatePhaseProgress(PHASE_CONNECTION_END, PHASE_FORMATION_END, animationTime);
        
        // Распад структуры мозга обратно в хаос (0 -> 1)
        // Важно: мы инвертируем это значение при отрисовке, чтобы линии исчезали
        const brainDissolve = calculatePhaseProgress(BRAIN_DISSOLVE_START, ANIMATION_DURATION, animationTime);

        // Обновляем позиции (они движутся всегда)
        particles.forEach(p => p.update());
        
        // Рисуем с учетом всех факторов
        drawConnections(connectionProgress, formationProgress, brainDissolve, masterOpacity);
        drawParticles(formationProgress, brainDissolve, masterOpacity);
    ...
    // 8. ФУНКЦИИ ОТРИСОВКИ
    // ============================================================
    function drawConnections(connProg, formProg, dissolveProg, masterOpacity) {
        ...
        for (let i = 0; i < particles.length; i++) {
            for (let j = i + 1; j < particles.length; j++) {
                ...
                if (distance < MAX_CONNECTION_DISTANCE) {
                    // Базовая прозрачность хаотичных линий
                    let baseOpacity = map(distance, 0, MAX_CONNECTION_DISTANCE, 0.3, 0);
                    
                    // Появление линий (connProg)
                    let opacity = baseOpacity * connProg;

                    // Усиление внутри мозга (formProg)
                    if (formProg > 0) {
                        const p1In = p1.isInsideBrain(), p2In = p2.isInsideBrain();
                        if (p1In && p2In) {
                            opacity = map(formProg, 0, 1, opacity, baseOpacity * 2.0);
                        } else if (!p1In && !p2In) {
                            opacity *= (1 - formProg * 0.9); // Внешние линии почти исчезают
                        }
                    }
                    
                    // Распад структуры мозга (dissolveProg)
                    // Структура исчезает, возвращаясь к прозрачности 0 или базовой
                    if (dissolveProg > 0) {
                         opacity *= (1 - dissolveProg);
                    }

                    // Глобальное исчезновение сцены
    ...
    function drawParticles(formProg, dissolveProg, masterOpacity) {
        if (masterOpacity < 0.01) return;

        particles.forEach(p => {
            let opacity = 1;
            
            // Затемнение внешних частиц при формировании мозга
            if (formProg > 0) {
                if (!p.isInsideBrain()) {
                    // Если мозг сформирован, внешние частицы тусклые
                    let dimFactor = 1 - formProg * 0.8;
                    
                    // НО! Если мозг начал распадаться (dissolveProg), внешние частицы снова могут стать видны
                    // (или просто следуем за masterOpacity). Для простоты оставим их тусклыми, 
                    // так как мы все равно уходим в fadeOut.
                    opacity = dimFactor;
                }
            }
            
            // Глобальное исчезновение
            opacity *= masterOpacity;
    ...
    function init() {
        ...
        // Подгон размера SVG под экран
        const desiredHeight = height * 0.6; // 60% высоты экрана
        const scale = desiredHeight / 3000; // 3000 - примерная высота viewbox SVG
        const scaledWidth = 3000 * scale;
        
        const matrix = new DOMMatrix();
        matrix.translate(width / 2 - scaledWidth / 2, height / 2 - desiredHeight / 2);
...
//     // ============================================================
//     // Мы увеличиваем общее время, чтобы добавить фазу затухания
//     // Было 20 сек (из них последние 2 сек - растворение).
//     // Теперь делаем 22 сек: 
//     // 0-18 сек - основная анимация
//     // 18-22 сек - отдельный процесс полного затухания
    
//     const PHASE_HOLD_END = 18;          // Конец фазы удержания мозга
//     const ANIMATION_DURATION = 22;      // Общее время петли (добавили 4 сек на плавное исчезновение)
    
//     const FADE_IN_DURATION = 2;         // Время появления из пустоты в начале (0-2 сек)
//     const FADE_OUT_START = 18;          // Начало затухания в конце (после удержания)
...
//         // --- ЛОГИКА БЕСШОВНОЙ ПЕТЛИ (MASTER OPACITY) ---
//         // 1. Появление в начале (0-2 сек), чтобы не было резкого скачка из пустоты
//         const fadeIn = calculatePhaseProgress(0, FADE_IN_DURATION, animationTime);
        
//         // 2. Исчезновение в конце (18-22 сек) - отдельный процесс
//         const fadeOut = calculatePhaseProgress(FADE_OUT_START, ANIMATION_DURATION, animationTime);
        
//         // Итоговая прозрачность всей сцены:
//         // Сначала растет от 0 до 1, держится, а в конце падает от 1 до 0.
//         const masterOpacity = fadeIn * (1 - fadeOut);

//         // Стандартные фазы формирования мозга
//         const connectionProgress = calculatePhaseProgress(PHASE_QUIESCENCE_END, PHASE_CONNECTION_END, animationTime);
//         const formationProgress = calculatePhaseProgress(PHASE_CONNECTION_END, PHASE_FORMATION_END, animationTime);

//         particles.forEach(p => p.update());
        
//         // Передаем masterOpacity во все функции рисования
//         drawConnections(connectionProgress, formationProgress, masterOpacity);
//         drawParticles(formationProgress, masterOpacity);
...
//     // ФУНКЦИИ ОТРИСОВКИ
//     // ============================================================
//     function drawConnections(connProg, formProg, masterOpacity) {
//         // Если мастер-прозрачность близка к 0, не нагружаем процессор расчетами
//         if (masterOpacity < 0.01) return;
...
//                     // Применяем мастер-прозрачность (затухание в конце)
//                     opacity *= masterOpacity;
...
//     function drawParticles(formProg, masterOpacity) {
//         if (masterOpacity < 0.01) return;
...
//             // Применяем мастер-прозрачность (появление в начале, исчезновение в конце)
//             opacity *= masterOpacity;
...
// // Этот код выполняется, когда вся страница (HTML) загружена (на первом месте)
// window.addEventListener('DOMContentLoaded', () => {
...
//     // ============================================================
//     // КОНФИГУРАЦИЯ ВРЕМЕНИ (в секундах) - ОБНОВЛЕННАЯ
//     // ============================================================
//     const ANIMATION_DURATION = 20;     
//     const PHASE_QUIESCENCE_END = 2;    // Движущиеся точки 0-2 сек
//     const PHASE_CONNECTION_END = 5;    // Прорастание линий 2-5 сек
...
// // Этот код выполняется, когда вся страница (HTML) загружена (пока более-менее версия)
// window.addEventListener('DOMContentLoaded', () => {
...
//     // ============================================================
//     // КОНФИГУРАЦИЯ ВРЕМЕНИ (в секундах) - БЕЗ ИЗМЕНЕНИЙ
//     // ============================================================
//     const ANIMATION_DURATION = 25;
//     const PHASE_QUIESCENCE_END = 2;
//     const PHASE_CONNECTION_END = 3.5;  // Быстрое соединение
//     const PHASE_FORMATION_END = 7;
//     const PHASE_HOLD_END = 18;         // Долгое удержание
//     const PHASE_DISSOLUTION_END = 25;
...
//     // -------------------------------------------------------------
//     // ИЗМЕНЕНО ЗДЕСЬ: Скорость уменьшена в 4 раза (было 0.2)
//     // -------------------------------------------------------------
...
//             this.y = y;
//             // -------------------------------------------------------------
//             // ИЗМЕНЕНО ЗДЕСЬ: Убран множитель *2, теперь скорость точно соответствует DRIFT_SPEED
//             // -------------------------------------------------------------
