...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // ============================================================
    // КОНФИГУРАЦИЯ ВРЕМЕНИ (в секундах)
    // ============================================================
    ...
    class Particle {
        constructor(x, y) { 
            ...
            // Уникальный ID для создания постепенного появления линий
            this.id = Math.random();
        }
        update() { 
            this.x += this.vx; 
    ...
    const easeOutQuad = t => t * (2 - t); // Более мягкое замедление
    ...
    // ============================================================
    // ФУНКЦИИ ОТРИСОВКИ - ПЛАВНОЕ ПРОРАСТАНИЕ
    // ============================================================
        ...
        for (let i = 0; i < particles.length; i++) {
            for (let j = i + 1; j < particles.length; j++) {
                ...
                if (distance < MAX_CONNECTION_DISTANCE) {
                    // Создаем уникальный "порог появления" для каждой пары точек
                    const connectionThreshold = (p1.id + p2.id) / 2;
                    
                    // Линия появляется постепенно, в зависимости от порога
                    let connectionVisibility = 0;
                    if (connProg > connectionThreshold) {
                        // Плавное появление конкретной линии
                        connectionVisibility = Math.min(1, (connProg - connectionThreshold) / (1 - connectionThreshold));
                        connectionVisibility = easeOutQuad(connectionVisibility);
                    }
                    
                    if (connectionVisibility < 0.01) continue;
                    
                    // Базовая прозрачность зависит от расстояния
                    let baseOpacity = map(distance, 0, MAX_CONNECTION_DISTANCE, 0.3, 0);
                    
                    // Начальная прозрачность с учетом видимости
                    let opacity = baseOpacity * connectionVisibility * 0.4;
                    ...
                    if (formProg > 0) {
                        ...
                        if (p1In && p2In) {
                            ...
                            const targetOpacity = baseOpacity * 1.4; // Увеличено с 0.85 до 1.4
                            opacity = opacity + (targetOpacity - opacity) * formProg;
                        } else if (!p1In && !p2In) {
                            // Линии снаружи плавно слабеют
                            opacity *= (1 - formProg * 0.7);
                        }
                    }

                    // Очень плавное растворение в конце
                    if (dissProg > 0) {
                        const easedDiss = easeOutQuad(dissProg);
                        opacity *= (1 - easedDiss);
                    }
    ...
    function drawParticles(formProg, dissProg) {
        particles.forEach(p => {
            ...
            if (formProg > 0) {
                if (!p.isInsideBrain()) {
                    const easedForm = easeOutQuad(formProg);
                    opacity = 1 - easedForm * 0.75;
                }
            }
...
//                         const p1In = p1.isInsideBrain(), p2In = p2.isInsideBrain();
//                         if (p1In && p2In) {
//                             // Линии внутри мозга плавно усиливаются
//                             const targetOpacity = baseOpacity * 0.85;
//                             opacity = opacity + (targetOpacity - opacity) * formProg;
//                         } else if (!p1In && !p2In) {
//                             // Линии снаружи плавно слабеют
//                             opacity *= (1 - formProg * 0.7);
