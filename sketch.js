window.addEventListener('DOMContentLoaded', () => {
    ...
    // ============================================================
    // ИДЕАЛЬНЫЙ ТАЙМИНГ (БЕСШОВНЫЙ ЛУП)
    // ============================================================
    const ANIMATION_DURATION = 19;     
    
    // Фазы анимации
    const PHASE_QUIESCENCE_END = 2;    // Хаос точек
    const PHASE_CONNECTION_END = 5;    // Появление линий
    const PHASE_FORMATION_END = 8;     // Мозг полностью сформирован
    const PHASE_HOLD_END = 15;         // Удержание формы мозга
    
    // Новая логика: Мозг начинает растворяться (линии и форма) ДО того, как исчезнет вся картинка.
    const BRAIN_DISSOLVE_START = 15;   // Мозг теряет структуру, возвращаясь к хаосу
    
    // Глобальное затухание (Fade Out) начинается, пока частицы еще движутся
    const FADE_OUT_START = 16;         
    
    // В начале анимации (0-2 сек) мы плавно проявляем сцену (Fade In)
    ...
    function animate() {
        ...
        // --- ЛОГИКА ПРОЗРАЧНОСТИ (MASTER OPACITY) ---
        // 1. Плавное появление в начале (0-2с)
        const fadeIn = calculatePhaseProgress(0, FADE_IN_DURATION, animationTime);
        // 2. Плавное исчезновение в конце (16-19с)
        const fadeOut = calculatePhaseProgress(FADE_OUT_START, ANIMATION_DURATION, animationTime);
        // Итоговая прозрачность всей сцены (от 0 до 1 и обратно в 0)
        ...
        // Растворение структуры (линии внутри начинают слабеть до того, как исчезнет вся сцена)
        ...
        for (let i = 0; i < particles.length; i++) {
            for (let j = i + 1; j < particles.length; j++) {
                ...
                if (distance < MAX_CONNECTION_DISTANCE) {
                    ...
                    // Если началась фаза растворения структуры мозга, линии внутри слабеют
                    if (dissolveProg > 0) {
    ...
    function drawParticles(formProg, dissolveProg, masterOpacity) {
        ...
        particles.forEach(p => {
            ...
            if (formProg > 0) {
                // В фазе формирования внешние точки тускнеют
                if (!p.isInsideBrain()) {
                    opacity = 1 - formProg * 0.8;
                }
            }
            
            // В фазе растворения структуры внешние точки могут снова стать ярче (возврат к хаосу)
            // или просто все уходит в masterOpacity. Здесь оставим простое затухание.
    ...
    function init() {
        ...
        const scale = desiredHeight / 3000;
