window.addEventListener('DOMContentLoaded', () => {
    ...
    // ============================================================
    // ТАЙМИНГИ (линии начинаются с 0 сек)
    // ============================================================
    const ANIMATION_DURATION = 20;     
    const PHASE_CONNECTION_END = 6;    // Линии уплотняются от 0 до 6 сек
    const PHASE_FORMATION_END = 10;    // Мозг окончательно собирается к 10 сек
    const PHASE_HOLD_END = 17;         
    const PHASE_DISSOLUTION_END = 20;  

    // ============================================================
    // ВИЗУАЛЬНЫЕ НАСТРОЙКИ (увеличено количество и дистанция)
    // ============================================================
    const GRADIENT_START_COLOR = '#faf9f6'; 
    const GRADIENT_END_COLOR = '#e8ede7';   
    const PARTICLE_COLOR = '#4A4A4A';       
    const NUM_PARTICLES = 550;             // Чуть больше точек
    const MAX_CONNECTION_DISTANCE = 140;   // ЗНАЧИТЕЛЬНО больше линий (было 100)
    ...
    class Particle {
        ...
        display(opacity = 1) { 
            ctx.fillStyle = PARTICLE_COLOR; ctx.globalAlpha = opacity; 
            ctx.beginPath(); ctx.arc(this.x, this.y, PARTICLE_SIZE, 0, Math.PI * 2); ctx.fill(); 
        }
        isInsideBrain() { 
            if (!brainPath2D) return false; 
            return ctx.isPointInPath(brainPath2D, this.x, this.y); 
        }
    }
    ...
    function animate() {
        ...
        // Линии теперь считаются от 0 секунды
        const connProg = calculatePhaseProgress(0, PHASE_CONNECTION_END, animationTime);
        const formProg = calculatePhaseProgress(PHASE_CONNECTION_END, PHASE_FORMATION_END, animationTime);
        const dissProg = calculatePhaseProgress(PHASE_HOLD_END, PHASE_DISSOLUTION_END, animationTime);

        particles.forEach(p => p.update());
        
        drawConnections(connProg, formProg, dissProg);
        drawParticles(formProg, dissProg);
        
        requestAnimationFrame(animate);
    }

    function drawConnections(connProg, formProg, dissProg) {
        // Даже при connProg = 0 даем минимальную видимость 0.1, чтобы линии были сразу
        let globalConnAlpha = map(connProg, 0, 1, 0.15, 1) * (1 - dissProg);
        ...
        for (let i = 0; i < particles.length; i++) {
            for (let j = i + 1; j < particles.length; j++) {
                ...
                if (distance < MAX_CONNECTION_DISTANCE) {
                    ...
                    let baseOpacity = map(distance, 0, MAX_CONNECTION_DISTANCE, 0.4, 0);
                    let opacity = baseOpacity * globalConnAlpha;

                    // Уплотнение внутри мозга
                    if (formProg > 0 && dissProg === 0) {
                        const p1In = p1.isInsideBrain(), p2In = p2.isInsideBrain();
                        if (p1In && p2In) {
                            opacity = map(formProg, 0, 1, opacity, baseOpacity * 1.5);
                        } else if (!p1In && !p2In) {
                            opacity *= (1 - formProg * 0.8);
                        }
                    }
                    ...
                    if (opacity > 0.01) {
                         ...
                         // ЛИНИИ УПЛОТНЯЮТСЯ: от 0.2px до 0.7px
                         ctx.lineWidth = 0.2 + (connProg * 0.5); 
                         ctx.beginPath();
    ...
    function drawParticles(formProg, dissProg) {
        particles.forEach(p => {
            let opacity = map(calculatePhaseProgress(0, 2, (Date.now() - startTime)/1000 % 20), 0, 1, 0.3, 1);
            if (formProg > 0 && dissProg === 0) {
                if (!p.isInsideBrain()) opacity = 1 - formProg * 0.8;
            }
            if (dissProg > 0) opacity *= (1 - dissProg);
            if (opacity > 0.01) p.display(opacity);
        });
    }
    ...
    function init() {
        ...
        matrix.translate(width / 2 - (3000 * scale) / 2, height / 2 - desiredHeight / 2);
...
// // Этот код выполняется, когда вся страница (HTML) загружена (этот вариант)
// window.addEventListener('DOMContentLoaded', () => {
...
//     // // ============================================================
//     // // КОНФИГУРАЦИЯ ВРЕМЕНИ (в секундах) - УЛУЧШЕННАЯ
//     // // ============================================================
//     // const ANIMATION_DURATION = 20;     
//     // const PHASE_QUIESCENCE_END = 3;    
//     // const PHASE_CONNECTION_END = 7;    
//     // const PHASE_FORMATION_END = 11;    
//     // const PHASE_HOLD_END = 16;         
//     // const PHASE_DISSOLUTION_END = 20;  
//     // ============================================================
//     // КОНФИГУРАЦИЯ ВРЕМЕНИ (в секундах) - УЛУЧШЕННАЯ 2 вариант
//     // ============================================================
//     const ANIMATION_DURATION = 20;     
//     const PHASE_QUIESCENCE_END = 2;    // Движущиеся точки 0-2 сек
//     const PHASE_CONNECTION_END = 5;    // Прорастание линий 2-5 сек
//     const PHASE_FORMATION_END = 8;     // Формирование мозга 5-8 сек
//     const PHASE_HOLD_END = 18;         // Удержание 8-18 сек (10 секунд)
//     const PHASE_DISSOLUTION_END = 20;  // Растворение 18-20 сек
    
//     // ============================================================
//     // НОВАЯ ВИЗУАЛЬНАЯ КОНФИГУРАЦИЯ (для градиента)
//     // ============================================================
//     const GRADIENT_START_COLOR = '#faf9f6'; // Ваш светлый песочный
//     const GRADIENT_END_COLOR = '#e8ede7';   // Гармонирующий светло-зеленый (шалфейный)
//     const PARTICLE_COLOR = '#4A4A4A';       // Темно-серый для контраста
...
//     // ============================================================
//     // ФУНКЦИИ ОТРИСОВКИ - ИСПРАВЛЕННЫЕ
//     // ============================================================
//     function drawConnections(connProg, formProg, dissProg) {
//         for (let i = 0; i < particles.length; i++) {
//             for (let j = i + 1; j < particles.length; j++) {
...
//                     // Плавное появление линий (фаза connection)
//                     let opacity = baseOpacity * connProg;

//                     // Плавное усиление внутри мозга (фаза formation)
//                     if (formProg > 0) {
//                         const p1In = p1.isInsideBrain(), p2In = p2.isInsideBrain();
//                         if (p1In && p2In) {
//                             // Линии внутри мозга плавно усиливаются до максимума
//                             opacity = map(formProg, 0, 1, opacity, baseOpacity * 1.8);
//                         } else if (!p1In && !p2In) {
//                             // Линии снаружи плавно слабеют
//                             opacity *= (1 - formProg * 0.8);
//                         }
//                     }
...
//             // Плавное затухание точек снаружи мозга
//             if (formProg > 0) {
//                 if (!p.isInsideBrain()) {
//                     opacity = 1 - formProg * 0.8;
//                 }
